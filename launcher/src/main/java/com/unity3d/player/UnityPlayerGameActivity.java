...
public class UnityPlayerGameActivity extends GameActivity
    ...
    private void setupNativeControls(View overlay) {
        ...
        ImageView btnBackLoading = overlay.findViewById(R.id.btn_back_loading);
        if (btnBackLoading != null) {
            btnBackLoading.setOnClickListener(v -> finish());
        }
    ...
    protected void onDestroy() {
        ...
        super.onDestroy();
        // Terminate the isolated :unity process after the system server has completed
        // the activity transition back to MainActivity, releasing camera & RAM cleanly.
        new Thread(() -> {
            try {
                Thread.sleep(800);
    ...
    protected void onResume() {
        ...
        // Staged frame initialization & smooth loading fade-out
        mFrameSyncHandler.removeCallbacksAndMessages(null);
        ...
        mFrameSyncHandler.postDelayed(() -> {
            if (tvLoadingSubtitle != null) {
                tvLoadingSubtitle.setText("Applying 3D frame overlay...");
            }
            updateFrame();
        }, 1200);
        mFrameSyncHandler.postDelayed(() -> {
        ...
        mFrameSyncHandler.postDelayed(mFrameSyncRunnable, 2400);
    }
